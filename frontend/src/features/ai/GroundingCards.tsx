...
export default function GroundingCards({ message, language, dir }: { message: GeminiMessage; language: 'en' | 'ar'; dir: 'ltr' | 'rtl' }) {
  ...
  if (grounding.kind === 'profit_and_loss') {
    ...
    return <div className="mt-2 space-y-2 rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3" dir={dir}><div className="flex flex-wrap items-start justify-between gap-2"><div><div className="font-semibold text-emerald-200">{text(ar, 'Profit and Loss', 'الأرباح والخسائر')}</div><div className="text-[11px] text-gray-400">{periodLabel}</div></div><span className="flex items-center gap-1 text-[10px] text-emerald-300"><CheckCircle2 className="h-3 w-3" />{text(ar, 'Verified from accounting data', 'تم التحقق من بيانات النظام')}</span></div><div className="space-y-1">{(['revenue', 'expenses', 'net_profit'] as const).map((key) => <div key={key} className={`flex w-full min-w-0 items-center justify-between gap-3 rounded-lg px-2 py-1.5 ${grounding.requested_metric === key ? 'bg-brand-500/15 ring-1 ring-brand-400/30' : ''}`}><span className="min-w-0 flex-1 break-words text-xs text-gray-400">{text(ar, key === 'revenue' ? 'Revenue' : key === 'expenses' ? 'Expenses' : 'Net profit', key === 'revenue' ? 'الإيرادات' : key === 'expenses' ? 'المصروفات' : 'صافي الربح')}</span><span className="shrink-0 whitespace-nowrap font-mono text-xs text-gray-100">{grounding.metrics[key]}</span></div>)}</div><button type="button" onClick={open} className="inline-flex items-center gap-1 rounded-lg border border-brand-500/30 px-2.5 py-1.5 text-xs font-semibold text-brand-300 hover:bg-brand-500/10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-400"><ExternalLink className="h-3.5 w-3.5" />{text(ar, 'Open Profit and Loss', 'فتح تقرير الأرباح والخسائر')}</button></div>;
  }
  const { total_matches: total, returned_matches: returned, has_more: more } = grounding.summary;
  return <div className="mt-2 space-y-2" dir={dir}><div className="flex flex-wrap justify-between gap-2 text-xs text-gray-400"><span>{total === 0 ? text(ar, 'No matching journal entries found.', 'لم يتم العثور على قيود مطابقة.') : text(ar, `${total} matching journal entries found.`, `تم العثور على ${total} قيود مطابقة.`)}</span>{more && <span className="text-amber-300">{text(ar, `Showing ${returned} of ${total}.`, `يتم عرض ${returned} من أصل ${total}.`)}</span>}</div>{grounding.entries.map((entry) => <article key={entry.journal_entry_id} className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-2.5 text-xs"><div className="flex items-start justify-between gap-2"><span className="min-w-0 truncate font-mono font-semibold text-brand-300" title={entry.entry_number}>{entry.entry_number}</span><span className="shrink-0 text-gray-500">{new Date(entry.entry_date).toLocaleDateString()}</span></div><div className="mt-1 truncate text-gray-200" title={entry.description || undefined}>{entry.description || text(ar, 'Not available', 'غير متوفر')}</div><div className="mt-1 grid gap-x-2 gap-y-1 text-gray-400 sm:grid-cols-2"><span>{text(ar, 'Status', 'الحالة')}: {status(entry.status, ar)}</span><span>{text(ar, 'Source', 'المصدر')}: {source(entry.source, ar)}</span><span>{text(ar, 'By', 'بواسطة')}: {entry.creator_name || text(ar, 'Not available', 'غير متوفر')}</span><span>{text(ar, 'Contribution', 'المساهمة')}: <b className="font-mono text-gray-200">{entry.matched_amount}</b></span><span className="sm:col-span-2">{text(ar, 'Match', 'سبب المطابقة')}: {reason(entry.match_reason, ar)}</span></div><button type="button" onClick={() => navigate(`/journal-entries?entry_id=${encodeURIComponent(String(entry.journal_entry_id))}`)} className="mt-2 inline-flex items-center gap-1 rounded-lg border border-brand-500/30 px-2.5 py-1.5 font-semibold text-brand-300 hover:bg-brand-500/10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-400"><ExternalLink className="h-3.5 w-3.5" />{text(ar, 'Open entry', 'فتح القيد')}</button></article>)}</div>;
}
